...
def load_clean_instances(filename):
	...
	with open('raw.csv', 'r') as f:
		...
		for line in lineIter:
			...
			# only look for year 2012
			if entry[0] == '2012':
				num_of_people = entry[-1]
				if num_of_people > '1':
					for p in range(int(num_of_people)):
						entry[-1] = 1;
...
clean_instances = load_clean_instances('agaricus-lepiota.data')
...
def majority_value(instances, class_index=0):
  ...
  max = 0
  for val in ('e', 'p'):   # should look up these values based on class_index using attribute_names_and_values
...
def create_decision_tree(instances, candidate_attribute_indexes=None, class_index=4, default_class=None, trace=0):
